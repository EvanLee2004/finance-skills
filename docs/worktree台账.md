...
| 金蝶入账 `kingdee-posting` | 正式 clone · `main`（已含 018） | 使用中 | 本机 json 分入口；环境管家已下线 |
| 收款入金蝶 `016` | `~/.grok/worktrees/skills-finance-skills/kingdee-receipt-016` | 可删 | 018 已覆盖收款主路径并进 main |
| 三模块合一 `017` 后续 | `~/.grok/worktrees/skills-finance-skills/kingdee-posting-unify` detached `9ddbb67` | 使用中 | 多条1131拆腿 / 点头新建客户。未并入 main |
| 收款少问能跑完 `018` | `/Users/evanlee/.grok/worktrees/skills-finance-skills/kingdee-receipt-018` · `kingdee-receipt-018` @ `207f5b2` | 可删 | 已进 GitHub `main`。json/环境管家改在正式 clone 上续 |
| 月度损益表 `pl-dept-report` `013` | 正式 clone · `main` | 使用中 | 薪酬台账 + 残差归集 |
