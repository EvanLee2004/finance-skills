...
> 一句话定位：金蝶星辰各账套科目余额 + 部门核算项目 + 利润表本月金额 → 斯佳「损益表 + 利润表」Excel；没账套的列留空。公式自写核对，禁止改数凑平。
...
| 3 | 没账套的（山东/四川/济南）空着或从代账利润表抄 | 容易填成 0 冒充已取 |
...
**她只需要做三件事**：能引出的账套引出放到一个文件夹 → 说「月度损益表」→ 打开两个 sheet 看核对列。

| 谁 | 做什么 |
|---|---|
| **她** | 把科目余额表 / 核算项目余额表 / 利润表引出放进一个文件夹（文件名随意），说「**月度损益表**」；可说期间 `202608` |
| **AI** | 总部只读 API 取甲骨易；另外 4 本星辰账本机浏览器引出 → 调 `convert.py` → 出两个 sheet + 运行报告 → **停下报路径** |
...
```mermaid
flowchart TD
  IN["她给文件夹：引出 xlsx<br/>或只说期间"] --> INS["inspect_inputs.py<br/>按 sheet/表头认<br/>不靠文件名"]
  KEY["本机 kingdee.local.json<br/>已在就直接用"] --> API["总部只读 API<br/>利润表 / 凭证降级"]
  ...
  F --> CHK{"核对非 0？"}
  CHK -->|"个数=0"| OK["交表"]
  CHK -->|"个数>0"| STOP["停下报科目编码个数<br/>禁止改数"]
```
...
| 输入 | 处理 | 输出 |
|---|---|---|
| 期间 YYYYMM（默认上月）；可选引出文件夹；本机密钥 | 总部 API 或引出；4 本星辰账引出；3 列无账套留空 | `月度损益表_YYYYMM.xlsx`（损益表+利润表）+ `*_运行报告.txt` |
...
- 两个 sheet、无确认情况、冻结 C3、山东/四川/济南无源则空
...
- 科目余额期间参数不通时总部改凭证 5 开头汇总；核算项目 OpenAPI 不通时凭证部门辅助
- 上月利润列没有源就空，禁止复制当月
