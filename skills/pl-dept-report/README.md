...
| 谁 | 做什么 |
|---|---|
| **她** | 说「**月度损益表**」；可说期间 `202608`。把山东/四川/济南三张代账利润表放进打开的文件夹或当天桌面夹。已有引出也可以随口给一个文件夹。网页账密只在本机缺文件时问一次 |
| **AI** | 先把总部 API 落成 `API/` 源 Excel，再网页引出缺的账套，再跑 `convert.py`。同一账套有网页文件就不用 API 那份叠两遍。**API 失败或金蝶账号登不上：不出表，把 `ask=` 原样问她。** 能取到数才出两个 sheet + 运行报告。斯佳交的损益表成品只对照，不当源。 |
| **她** | 打开表看列顺序、空列、核对公式；口径要调就改 `config/` 再跑。没出表就先把账密/引出补上再跑 |

**她会看到的话长这样**：

```
月度损益表已出，期间 202608。有源账套：甲骨易,文化；缺源：上海,山东分公司,…。利润表缺源：无。利润表上月缺线下：山东分公司,四川分公司,济南子公司。未映射部门 N 个。核对非 0 的科目编码 M 个。表在 <路径>。
```

登不上时不要说「已出」，只把 `ask=` 问出来，例如：本机没有金蝶网页账密 / API 取数失败。
...
flowchart TD
  ...
  B --> B1{"API或金蝶账号登得上?"}
  B1 -->|否| Z["停下来问人，不出表"]
  B1 -->|是| C["代账利润表 + 职工薪酬台账 / 薪酬拆分表"]
  C --> C1{"台账怎么来的？"}
  C1 -->|夹子里扫到·文件名假数/明昊测试| C3["不当薪酬源"]
  C1 -->|--payroll-xlsx 亲口指| C4["一定吃；sheet 名去数字认；认得但没金额写进报告"]
  C1 -->|夹子里扫到·正常名| C4
  C4 --> C5{"湖南台账合计 == 金蝶整笔?"}
  C5 -->|是| C6["左列按台账拆 5401/5502"]
  C5 -->|否| C7["金蝶不动，报告 未重分"]
  C6 --> C2["残差归集 + 济南服务费 + 房租摘要"]
  C7 --> C2
  C3 --> C2
  C2 --> D["脚本拼成两个 sheet：总部父行抄金蝶，其余父行=子行 SUM 公式；父级整笔无叶子 → ask"]
  D --> E{"运行报告"}
...
# 缺四本引出、或利润表只有上期文件时，脚本会调 xingchen_export.py（查询页选当期，不点生成）
```
...
- 运行报告 `核对非0其它` 必须是无；夹子里扫到的假数/明昊测试文件名不当薪酬源（`--payroll-xlsx` 亲口指的例外）
- 没有真台账时先在她机器上跑 `scripts/payroll_ledger.py --profile <台账> --period YYYYMM`（零金额、零人名，只报每个 sheet 认成什么、有没有金额、哪些部门对不上）；`--split-table <out.xlsx>` 出部门级「薪酬拆分表」（主体/科目编码/项目/部门/次序/金额，无人名），这张表可以直接当 `--payroll-xlsx` 喂给技能
...
- 湖南分/子：金蝶 5602 整笔工资/社保/公积金，台账同项合计 == 金蝶时按台账拆到成本/管理/销售两侧（左右列都拆，报告 `薪酬左列重分=`）；不等就保持金蝶、报告 `薪酬左列未重分=`（斯佳 2026-09-14）
- 父行左列：总部抄金蝶，其余主体 = 子行 SUM 公式（斯佳 2026-09-14）。金蝶父级整笔无叶子 → 报告 `父行发生额未落叶子=` + `ask=`，不算平
- 上月利润列没有源就空，禁止复制当月。当期利润表走查询页本月金额，过期文件不挡拉取，禁止点新增/生成
- 有科目余额但利润表空：第一段写 `利润表缺源=`，并进 `ask=`，不要报 8 家齐了
- API 失败、没有应用号又没有总部引出、网页账密没有或登录/引出失败：`产物=未生成`，不出 xlsx，只问人
- 总部工资/社保拆分 ask：只有台账覆盖了甲骨易才抹掉；外地台账不能当总部已拆
