...
> 出纳每天 T-1 回款核销：**智云取数 → SOD 级判定 → 《核销日清》（盈亏+流转预览）→ 她确认一次 → 先盈亏明细、后流转安全子集**。
> 全程人在环，**永不碰智云**，金额只由 Python 脚本算。未确认不写表。
...
| 她确认后 | apply 写盈亏「明细」副本，并对本批实际写入订单逐字段回读 | 写好的副本 + `订单写入差异_<核销日>.xlsx` |
