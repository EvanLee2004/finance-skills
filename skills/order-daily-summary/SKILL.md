...
《下单数据》（行=日期 列=部门）· **《统计区间》**（这次算了哪几天 + 有没有断档）· 《处理日志》· 可选《明细》。

**跑完必须汇报**：统计了哪几天、总计与分部门、有没有未匹配销售、**《统计区间》页有没有红色断档提示**。

> **不认法定节假日**：长假后第一个上班日只抓"昨天"。程序会自己把漏掉的工作日印在《统计区间》页。
> 补断档**只用离线模式**喂一张覆盖那几天的导出表（`--from-xlsx <表> --no-date-filter`）；
> ⛔ **禁止逐天 `--today` 补跑**——窗口按运行日倒推会重叠，同一天算好几遍。
