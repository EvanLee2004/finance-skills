...
| **每天（自动）** | **WorkBuddy 定时任务 09:00 自动跑**（提示词见 `references/WorkBuddy定时任务提示词.md`） |
| **她（按需）** | 也可对 opencode 说「**跑下单汇总**」／「**九点下单**」手动跑、补数、对数 |
...
| 输入 | 处理 | 输出 |
|---|---|---|
| 智云「下单」表（在线抓）+ `config/销售组织架构.xlsx` | 工作日窗口 + 部门归属 + 万元换算 + **覆盖登记** | 「下单数据(万元)」xlsx：《下单数据》/**《统计区间》**/《处理日志》(/《明细》) |
...
- 单测 **31 例**（纯函数、不连网；金标那条本机无文件时优雅跳过）
- **内网真机复测通过（2026-07-24）**
...
- 内网地址与数据不外传
- **只出数不解读**：不改智云、不发报表
- **不认法定节假日**（明昊 2026-07-25 决定不在代码里处理）：长假后第一个上班日只抓"昨天"。
  但程序会**自己发现断档**并把漏掉的工作日红字印在《统计区间》页，照提示补一次即可。
  ⛔ 补断档别逐天跑（窗口按运行日倒推、会重叠，同一天算好几遍——实测验证过）
