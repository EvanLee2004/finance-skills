...
```mermaid
flowchart TD
  IN["她说序时账入金蝶<br/>可带起始凭证号"] --> FIND["扫桌面 / Downloads / 她给的文件<br/>inspect 按表头认"]
  FIND --> KIND{"源表种类"}
  KIND -->|"凭证导入模板"| C["convert.py 转录"]
  KIND -->|"凭证列表#某期"| C
  KIND -->|"已经是官方引入表"| STOP["停下：不用再转"]
  C --> RULES["附件列空<br/>红字留在借方<br/>不合并不同凭证号"]
  RULES --> X["桌面 序时账入金蝶_今天/<br/>凭证引入_结果.xlsx"]
  X --> HUMAN["她去金蝶：查凭证 → 引入 → 新增<br/>只保存，不审核不过账"]
```
