...
三个入口共用一份客户核算项目余额表（`assist_xlsx` + `pick_assist_account`）抄 1131。同事机器**她自己引出**这张表（客户 + 1131 应收账款 + 本年）最稳；脚本自动引出受金蝶单点登录限制，只当备选。表若不是总部账套或没有 1131 明细，整批停并说明。不看下单号/合同号，不用智云业务线，不扫凭证往来窗口。没说凭证号时 = 现网当月最大 + 1，查询失败整批停。一个夹里几种表就一次跑几种（凭证号接着编），默认出到桌面 `金蝶入账_YYYYMMDD/销项|付款|收款/`；只一种表时仍是单独的桌面夹。
...
每夹：`凭证引入_<模块>_结果.xlsx`、`<源表名>_<模块>_明细结果.xlsx`、`对照说明_<模块>.md`（模块=销项/付款/收款）（只报笔数、原因类别、sheet、起始凭证号、余额表文件名）。
...
```mermaid
flowchart TD
  A["说：销项 / 付款 / 收款入金蝶"] --> B["按表头认哪一块"]
  B --> C["本机凭据拉档案和凭证号"]
  C -->   D["一条1131抄表；多条按申请人科目拆腿（客户销售例外优先）"]
  D --> E["不唯一或对不上进明细带候选"]
  E --> F["她去金蝶点引入"]
```
...
| 科目 | 抄余额表 1131；一条用；多条用申请人科目.json拆腿（须表里已有）；没有问新建。收入 5101+后两位 | 专票：540103+21710101 / 贷 100206。普票：540103 全额 / 贷 100206 | 贷方同销项；多条用销售走同一张人→尾号表 |
| 部门 / 人 | 组织架构优先，否则 `申请人部门.json`，再职员档案唯一部门。职员无档/一对多 hold | 0405 + 项目总监（须档案核验） | 表上部门编码 → 组织架构 → 申请人部门.json → 职员档案唯一部门 |
| 销售 | （申请人当职员） | — | 表上有则用；空则智云回款→下单（去有限公司/收缴户或唯一包含；回款还可按到账日+金额）。没智云仍出明细 |
| 特殊 | 公安除陈霞+海淀外→0386；人名→个人 | 无档/一对多→9999；PDF 缺票种/销售方/金额该家 hold | 海淀分局**不并** 0386。职员对不上留空仍入账 |
| 记账日 | 当前月最后一张凭证的日期（三入口同） | 同 | 同（斯佳 2026-09-14；表上收款日只用来查余额表期间） |
| 打包 | 约 5 张一记，连续同公司不拆 | 一家一记 | 约 10 笔一记，连续同公司不拆，换日另记 |

**闸门**：金蝶档案失败、没有应用号或当月凭证号查不到 → 不出引入表，stdout 的 `ask=` 原样问人。智云只给收款补空销售；失败不得整批空表。
...
| 付款 | 三列表 + 一家一个夹（夹里 PDF；夹名与台账差个括号也认，两个像的不猜） | 科目固定；无档/一对多→9999；PDF 读不出字单独说明；没装 pdfplumber 整批 `ask=` | 桌面 `金蝶入账_付款_YYYYMMDD/`（桌面在 OneDrive 下也能找到） |
| 收款 | 月底稿或收款表（中行收款 sheet） | 贷方抄余额表；表上销售优先，空则回款→下单；「类型」含保证金或 `--deposit-customer` → 贷 113312 投标保证金（客户 + 运营保障中心） | 桌面 `金蝶入账_收款_YYYYMMDD/` |
...
| `config/申请人科目.json` | 申请人/销售 → 应收后两位；只在余额表多条 1131 时拆腿；`_客户销售` 先于人名默认 |
| `config/收款不记.json` | 斯佳点头不记的银行抬头；进待确认，不新建 |
...
没给 `--out-dir` 时按模块写到上面三份桌面夹。可加 `--date YYYY-MM-DD`（默认 = 当前月金蝶最后一张凭证的日期，三入口统一；账上没凭证才用今天）。`--no-api` 会明确失败。合成测试可加 `--master` / `--lookups` JSON，生产不要用假档案出表。
...
- 收款：读中行收款或单独收款表；表上销售优先；空则回款（名称或到账日+金额）再下单；海淀不并 0386；没智云仍出明细；记账日=当前月最后一张凭证日期；部门可回退职员档案
- 无金蝶应用号、档案失败或当月凭证号查不到时不得出引入表。智云失败不是停表理由

## 10. 数据红线与已知边界

- 真实发票、台账、密钥不进本仓（GitHub/Gitee 是公开仓）
- 不点引入 / 审核 / 过账；不写智云。新建客户须斯佳点头后 `--create-new-customers`
...
工作台进**总部**账套后：财务会计 → 总账 → **凭证引入**。选技能交出的 `凭证引入_<模块>_结果.xlsx`，由她点「开始引入」。技能不代替这一步。
