...
**科目（斯佳 2026-09-07）**：发票不看「下单号」「合同号」，**不用智云业务线**。特殊规则先走（公安除陈霞+海淀外归 0386）。然后抄**客户核算项目余额表**：这家只有一条 1131 就用；两条期末有数（或期末都空但本年累计不唯一）问斯佳，禁止取最大；期末都空且本年累计只剩一条用那条；表里没有问是否新建，不编 KH*。收入 5101+后两位。OpenAPI 没有这张表，认已有 xlsx 或网页引出。凭证号没说时 = 当前月最大号 + 1。收款销售仍先对智云**回款记录**，没有再对**下单**。
...
flowchart TD
  ...
  S4 --> S6["申请人 → config 部门表 → API 核验编码<br/>约 5 张一记；凭证号=当月最大+1"]
  P --> P1["专票 540103+21710101 / 贷 100206<br/>普票 540103 全额；无档→9999"]
  R --> R1["贷方科目：金蝶 1131xx 往来（收款仍走旧路径）"]
  R1 --> R2{"销售：先回款记录"}
  R2 -->|唯一| R4["写入职员"]
  R2 -->|没有| R3{"再按下单客户找销售"}
  R3 -->|唯一| R4
...
| 模块 | 她给什么 | 技能补什么 | 输出 |
|------|----------|------------|------|
| 销项发票 | 发票簿：单位名称、价税合计、申请人（可无发票号、组织架构 sheet、科目列、下单号） | 客户核算项目余额表→应收/收入；申请人→部门 | `凭证引入_结果.xlsx` + 明细 |
