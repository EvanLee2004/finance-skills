...
一个技能、三个入口。金额、借贷、科目只由 `scripts/convert.py` 算。销项科目抄客户核算项目余额表；收款销售先回款再下单。对话只报笔数和路径，禁止报客户名、金额明细。
...
2. 禁止编 KH* / GYS*。销项按客户核算项目余额表对名称（去有限公司/写两遍/唯一包含），表里没有问斯佳是否新建（不调保存 API）。付款/收款仍对档案；2–5 个候选从本次档案里挑。公安除陈霞+海淀分局外归 0386。
3. 禁止点金蝶「开始引入 / 审核 / 过账」。
...
7. 禁止看发票「下单号」「合同号」。禁止用智云业务线或下单额定科目。销项科目只抄客户核算项目余额表（特殊规则先走；一家一条 1131 就用；两条期末有数则问斯佳；表里没有问是否新建）。不要扫凭证往来窗口、不要当月本期借方取大。
...
没有密钥、金蝶档案失败（含 errcode 1030002006 且主动获取授权仍失败）或本机缓存过期：**不生成引入表**。AppSecret 会每日刷新，技能用官方 `push_app_authorize` 更新本机文件，只认总部账套，**不要去开放平台点重置密钥**。销项不要求智云。收款才需要智云找销售。不能用 `--no-api` 绕过闸门。
...
- 收款/销项含「公安」：陈霞+海淀分局走自己的档案，其余归 0386。
- 客户档案 2–5 候选 → 停，让人从名单里挑；0 个 → 问斯佳是否新建，不要自己建档。
- 本机没有金蝶应用号、查档失败或缓存过期 → 停止，不交空编码的引入表。销项不要因为没智云就停。
