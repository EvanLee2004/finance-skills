...
一个技能、三个入口。金额、借贷、科目只由 `scripts/convert.py` 算。销项科目来自金蝶该客户 1131xx 往来；收款销售先回款再下单。对话只报笔数和路径，禁止报客户名、金额明细。
...
2. 禁止编 KH* / GYS*。客户名先脚本模糊（去有限公司/写两遍/唯一包含）；2–5 个候选从本次档案里挑，0 个停下来问斯佳是否新建（本技能不调保存 API）。公安除陈霞+海淀分局外归 0386。
3. 禁止点金蝶「开始引入 / 审核 / 过账」。
