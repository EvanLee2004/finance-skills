...
对 opencode 说下面任一句（两版或多版 list 放一起）：

- **推荐**：对比一下追觅这两版进度
- 也可：追觅对账 / 追觅应收进度对比 / 这几版 list 有啥变化
...
| 步 | 她手工怎么做 | 痛在哪 |
|---|---|---|
| 1 | 两个版本的 Excel 并排开着 | 列结构可能对不齐（新增了期间列） |
| 2 | 一行行对人名，找同一个人在两版里的位置 | **人名写法会变**（多空格、简称），肉眼对齐容易错位 |
| 3 | 逐格看金额变没变 | 几百格，看串行 |
| 4 | **还要看底色变没变**（底色代表状态） | 颜色变化比数字更难用眼睛扫 |
| 5 | 记下"谁催了、谁回款了、谁收了 PO" | 全靠人脑总结 |

> 最容易漏的是**底色变化**和**新增的期间列**。

## 现在怎么用（人 + AI 怎么配合）

| 谁 | 做什么 |
|---|---|
| **她** | 说「**对比一下追觅这两版进度**」，或直接把几个版本丢一个目录 |
| **AI** | 认文件（按文件名日期排序，旧→新）→ 人名对齐 + 期间并集 → 值 diff + **底色 diff** |
| **AI** | 出对比报告 + **人话结论摘要**（谁催了、谁回款了、谁收了 PO、有没有新月账） |
| **她** | 看结论摘要；要看细节就翻明细页签 |
...
```mermaid
flowchart TD
  A["说：对比追觅这两版"] --> B["对人名 + 期间"]
  B --> C["比值和底色"]
  C --> D["交出报告 + 人话结论"]
  D --> E["她看谁催了、谁回款"]
```

**口径**：预计付款列的**公式当备注忽略**（只比值不比公式）；人名智能对齐，对不上的单独列出来。
